...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Run patch-level DINO+OT matching and export top-k JSON/CSV files."
    )
    p.add_argument("--source_dir", type=Path, required=True, help="Source patch directory")
    p.add_argument("--target_dir", type=Path, required=True, help="Target patch directory")
    p.add_argument("--output_dir", type=Path, required=True, help="Output directory for top-k files")
    p.add_argument("--topk", type=int, default=10, help="Top-k matches per source patch")
